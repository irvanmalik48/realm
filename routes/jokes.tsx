...
export const handler: Handlers<JokesType> = {
  async GET(req, ctx) {
    ...
    const res: {
      ...
      text: await (await fetch("https://candaan-api.vercel.app/api/text/random")).json(),
      img: await (await fetch("https://candaan-api.vercel.app/api/image/random")).json(),
    };
    const cases = {
      _default: res,
      text: {
    ...
    const body: JokesType = includeParams !== null && includeParams !== "all" ? cases[includeParams as keyof typeof cases] : cases["_default"];
...
export default function Jokes(props: PageProps<{ text?: JokesText, img?: JokesImage }>) {
  return (
    <DefaultLayout
...
function TextJoke(props: { data: JokesText | undefined }) {
  return <div>
    ...
    <p className={tw`text-dark-text text-center px-5 py-3 text-lg bg-dark-accent-semitrans rounded-xl`}>
      {props?.data?.data}
    </p>
  </div>;
}
...
function ImageJoke(props: { data: JokesImage | undefined }) {
  return <div>
    ...
    <img className={tw`w-full h-[500px] object-contain bg-dark-superdark transition-all duration-200 ease-linear ring ring-transparent hover:ring-dark-accent-solid rounded-xl`} src={props?.data?.data.url} alt="joke" />
  </div>;
}
