  ...
  return (
    <div class="p-4 bg-neutral-800 rounded border border-neutral-700 not-prose flex flex-col gap-5">
      <div class="border border-neutral-700 p-3 rounded">
        <label for="link" class="text-neutral-100">Paste Link</label>
        <div class="w-full flex gap-3">  
          <pre class="w-full bg-neutral-900 border border-neutral-700 px-4 py-2 rounded" id="link">
            <a href={pasteLink() ?? "#"} class="w-full block text-center">
              {pasteLink() ?? "Your link will appear here"}
            </a>
          </pre>
          <button class="bg-neutral-900 border border-neutral-700 w-fit px-4 py-2 rounded bg-opacity-20 hover:bg-opacity-100 transition" onClick={() => {navigator.clipboard.writeText(pasteLink() ?? "")}}>
            Copy
          </button>
        </div>
      </div>
      <div class="border border-neutral-700 p-3 rounded">
        <label for="language" class="text-neutral-100">Language</label>
        <select class="bg-neutral-900 focus:ring-red-400 ring-transparent focus:outline-none ring-2 appearance-none border border-neutral-700 w-full px-4 py-2 rounded bg-opacity-20 hover:bg-opacity-100 transition" name="language" id="language">
      ...
      <textarea placeholder="Please copy your code here" style={{ "overflow-wrap": "normal" }} id="paste" name="paste" class="resize-none scrollbar-thin scrollbar-track-neutral-700 scrollbar-thumb-neutral-800 scrollbar-corner-neutral-700 whitespace-pre overflow-scroll w-full bg-neutral-900 rounded border border-neutral-700 px-4 py-2 transition min-h-[350px] focus:ring-red-400 ring-transparent focus:outline-none ring-2 font-mono" />
      <button class="bg-neutral-900 border border-neutral-700 w-full px-4 py-2 rounded bg-opacity-20 hover:bg-opacity-100 transition" type="submit" value="Submit" onClick={handleSubmit}>
        Submit
      </button>
    </div>
  )
