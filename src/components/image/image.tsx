...
const processImage = server$(async (imgUrl: string) => {
  const blurhashFetch = await fetch(`${imgUrl.split("?")[0]}?fm=blurhash`).then(
    (res) => res.text()
  );
...
export const Image = component$<ImageProps>((props: ImageProps) => {
  ...
  useTask$(async () => {
    blurHashData.value = blurhashToCssGradientString(
      await processImage(props.src)
    );
  });

  return <OptimizedImage {...props} background={blurHashData.value} />;
});
