...
export default function ProjectCard(props: {
  ...
  return (
    ...
    >
      <img
        src={asset(props.img as string)}
        alt="project image"
        className={tw`w-full h-auto`}
      />
      <div className={tw`pb-3`}>
        <p
          className={tw`text-dark-accent-solid text-lg pt-3 px-5 font-semibold font-heading`}
        >
          {props.title}
        </p>
        <p
          className={tw`text-dark-text px-5 text-sm mb-1 overflow-clip max-h-[1.25rem] ${
            lineClamp(1)
          }`}
        >
          {props.desc}
